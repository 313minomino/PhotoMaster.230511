...
class ViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    func drawMaskImage(image: UIImage) -> UIImage {
        let maskImage = UIImage(named: "furo_docky")!
    ...
    @IBAction func onTappedUploadButton() {
        if photoImageView.image != nil {
            let activitiVC = UIActivityViewController(activityItems: [photoImageView.image!,"#PhotoMaster"], applicationActivities: nil)
            self.present(activitiVC, animated: true, completion: nil)
        } else {
            print("画像がありません")
        }
        
        func onTappedIllustButton() {
            if photoImageView.image != nil {
